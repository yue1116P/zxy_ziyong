...
# chrome截图整个页面
- 在 Chrome 浏览器中截图整个网页最快的方法是使用【开发者工具】，按 F12 (Mac 可能是 Fn+F12) 打开，然后按 Ctrl+Shift+P (Mac: Cmd+Shift+P)，输入 Capture full size screenshot 即可自动保存网页长图。
---

# 安装mozc输入法
- https://blog.csdn.net/sweeper_freedoman/article/details/89930454
- 默认使用的fcitx5管理器，安装5版本
---
...
```
## 连接
- ssh -i ~/.ssh/id_rsa zhao@133.xx.xx.xx
---
...
# FileZilla FTP远程文件传输
## 安装
- https://blog.csdn.net/weixin_41010198/article/details/109054895
- 方法1：使用apt-get安装
  - sudo apt-get install filezilla
  - 安装本地化多语言包：sudo apt-get install filezilla-locales
- 方法2：下载安装包
  - tar文件解压
  - 解压后进入安装目录：cd FileZilla3/bin
  - 执行filezilla安装可执行文件：sudo ./filezilla
  - 创建桌面快捷方式：sudo vi /usr/share/applications/filezilla.desktop
## 使用
- 左上角选择SFTP连接
- 可以选择密钥登陆
- 连接后可以直接管理本地和服务器上文件的互传

# 其他杂项(备忘用)
## 查询iter与epoch对应关系
- 查看目录里文件创建时间,看验证集图片输出时间和那个iter最接近
- ls -la --time-style=full-iso .../visualization/ | grep "e001[0-9]"
- ls -la --time-style=full-iso .../models/
