...
- Latent Feature输入到“图像优化模块”中，并通过Text Embeddings特征矩阵进行优化的“控制”<br>
...
- 由一个U-Net网络和一个Schedule算法共同组成
  ...
  - Schedule算法对每次U-Net预测的噪声进行优化处理（动态调整预测的噪声，控制U-Net预测噪声的强度），从而统筹生成过程的进度。<br>
<img width="1000" height="500" alt="image" src="https://github.com/user-attachments/assets/995a9a41-cb91-4fd1-a34c-04392d9ca3cf" />  

3.图像解码器
- 图像优化后，将优化迭代后的Latent Feature输入到图像解码器（VAE Decoder）中，将Latent Feature重建成像素级图像。<br>
<img width="1000" height="500" alt="image" src="https://github.com/user-attachments/assets/3c035cac-7dd9-4796-a881-9d65a2febf92" /><br>
...
2.详解
- 无论是前向扩散过程还是反向扩散过程都是一个参数化的马尔可夫链（Markov chain）[原论文](https://link.zhihu.com/?target=https%3A//arxiv.org/pdf/2006.11239.pdf)<br>
<img width="1440" height="276" alt="image" src="https://github.com/user-attachments/assets/35479783-a88a-4f93-8db5-4ee11fabf6e6" /><br>
- 前向扩散过程是固定的，由Schedule算法进行统筹控制
  - 可以基于初始数据X0和任意的扩散步数Ki,采样得到对应的数据Xi
- 反向扩散过程中每一步预测并去除的噪声分布，都需要扩散模型在训练中学习。
- 训练目标：将扩散模型每次预测出的噪声和每次实际加入的噪声做回归，让扩散模型能够准确的预测出每次实际加入的真实噪声。<br>
<img width="720" height="992" alt="image" src="https://github.com/user-attachments/assets/75ace0f9-4641-4598-b2a3-4c5a47eca989" /><br>
- Latent的引入
  - 基于Latent的扩散模型可以将前向扩散过程还是反向扩散过程压缩在低维的Latent隐空间，大大降低了显存占用和计算复杂度
